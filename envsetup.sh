...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^bee_") ; then
       bee_BUILD=$(echo -n $1 | sed -e 's/^bee_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $bee_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       bee_BUILD=
    fi
    export bee_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    bee_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/bee/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var bee_VERSION)
        ZIPFILE=bee-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.bee.device=$bee_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $bee_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $bee_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/bee/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
